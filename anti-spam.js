...
module.exports = function (bot, options) {
  ...
  const warningMessage = (options && options.warningMessage) || "stop spamming or you will be warned/muted.";
  const banMessage = (options && options.banMessage) || "has been muted for spamming for 5 minutes, anyone else?";
  ...
  function mute(msg, userid) {
    ...
    if (user) {
      user.addRole(role).catch(function () {
        msg.channel.send("Come back with a permission nub");
        return false;
      });

...
module.exports = class RelationCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'relation',
            group: 'rp',
            ...
            args: [
                {
                    key: 'arg',
                    prompt: 'What are u trying to do?',
                    type: 'string'
                }
            ]
        });
        ...
        switch (args[0]) {
         ...
         console.log(member);
          ...
          requestList.forEach(
            ...
            break;
          case "accept":
          var membermention = `<@${message.mentions.users.first().id}>`;
          var memberargs = args[1].replace('@!','@');
                ...
                requestList.forEach(
                  function (item,index) {
                    if(item['requestedUserid'] === message.author.id && item['relation'] === args.slice(2).join(' '))
                    {
                      ...
                      let newRequestList=bot.relationRequest.get(message.mentions.users.first().id).filter(function( obj ) {
